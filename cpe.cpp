...
private:
    ...
    bool comparar(int posPadre, int pos)
    {
        ...
        if (vec[posPadre]->prioridad > vec[pos]->prioridad)
            intercambia = true; // caso > prioridad
        else if (vec[posPadre]->prioridad == vec[pos]->prioridad)
        {
            if (!vec[posPadre]->paraLlevar && vec[pos]->paraLlevar)
                intercambia = true; // caso paraLlevar
            else if (vec[posPadre]->paraLlevar == vec[pos]->paraLlevar)
            {
                if (vec[posPadre]->id > vec[pos]->id)
                    intercambia = true; // caso > id
            }
        }
        return intercambia;
    }
